...
def process_file(path, era_label):
    ...
    with uproot.open(path) as f:
        ...
        hlt_pnet_ditau = tree["HLT_IsoMu24_eta2p1_PNetTauhPFJet30_Medium_L2NN_eta2p3_CrossL1"].array()
        hlt_dtau_ditau = tree["HLT_IsoMu24_eta2p1_MediumDeepTauPFTauHPS35_L2NN_eta2p1_CrossL1"].array()
        match_pnet = tree["trig_match_PNet_DiTau_Loose"].array()
        match_deep = tree["trig_match_DeepTau_DiTau"].array()
    ...
    for i in tqdm(range(len(pt_2)), desc=f"{era_label}", unit="evt"):
        if pass_probe[i] and hlt[i] and met_filter[i] and abs(eta_2[i]) < 2.3 and trig_match_single_muon[i]:
...
eff_deep.SetTitle("DeepTau ditau Trigger Efficiency (2024)")
eff_deep.GetXaxis().SetTitle("Offline p_{T}^{#tau} [GeV]")
eff_deep.GetYaxis().SetTitle("Efficiency")

ratio.SetName("ratio_2024")
ratio.SetTitle("ParticleNet / DeepTau Efficiency Ratio (2024)")
