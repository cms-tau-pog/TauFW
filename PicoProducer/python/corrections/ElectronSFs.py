...
class ElectronSFs:
  ...
  def __init__(self, era, sf_id=None, sf_trig=None, flags=[ ], verb=0):
    ...
    if (not fname_id or not os.path.exists(fname_id)) and (not fname_id_wpiso or not os.path.exists(fname_id_wpiso)):
    ...
    # LOAD CORRECTIONS
    if "2024" in era:
      corrset_id_wpiso = CorrectionSet.from_file(fname_id_wpiso)
      corrset_id_reco = CorrectionSet.from_file(fname_id_reco)
      self.sftool_id_wpiso  = getcorr(corrset_id_wpiso,sf_id,verb=verb)
      self.sftool_id_reco  = getcorr(corrset_id_reco,sf_id,verb=verb)
    else:
