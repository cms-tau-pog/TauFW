...
class PileupWeightTool:
  ...
  def __init__( self, year=2017, sigma='central', sample=None, buggy=False, flat=False ):
    ...
    if year==2016:
      datafilename = os.path.join(datadir,"Data_PileUp_2016_%s.root"%(minbias))
      mcfilename   = os.path.join(datadir,"MC_PileUp_2016_Moriond17.root")
    elif year==2017:
      tag = ""
      if buggy or sample:
      ...
      datafilename = os.path.join(datadir,"Data_PileUp_2017_%s.root"%(minbias))
      mcfilename   = os.path.join(datadir,"MC_PileUp_2017_Winter17_V2%s.root"%(tag))
    else:
      datafilename = os.path.join(datadir,"Data_PileUp_2018_%s.root"%(minbias))
      mcfilename   = os.path.join(datadir,"MC_PileUp_2018_Autumn18.root")
