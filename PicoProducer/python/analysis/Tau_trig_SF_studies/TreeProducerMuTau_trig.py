...
# Changes made for MuTau_trigger studies: Athar Ahmad (Nov 2025)
# Sources:
...
class TreeProducerMuTau_trig(TreeProducerTauPair):
  """Class to create and prepare a custom output file & tree."""
  
  def __init__(self, filename, module, **kwargs):
    print("Loading TreeProducerMuTau_trig for %r"%(filename))
    super(TreeProducerMuTau_trig,self).__init__(filename,module,**kwargs)
    
    # Extra Filterbits
    self.addBranch('pass_L2NN', '?')
    self.addBranch('pass_PNet_Tauh', '?')
    self.addBranch('pass_MuTau_overlap', '?')
    ...
    self.addBranch('trig_match_PNet_DiTau_Flat', '?', False)
    ...
    self.addBranch('trig_match_PNet_ETau_Loose_2025E', '?', False)
    self.addBranch('trig_match_PNet_ETau_Medium_2025E', '?', False)
    self.addBranch('trig_match_PNet_ETau_Tight_2025E', '?', False)
