...
parser = argparse.ArgumentParser(description="Combined Tau Trigger Efficiency Plotter")
...
for i in range(len(pt_2)):
    if pass_probe[i] and hlt_24[i]:
        hist_total_pt.Fill(pt_2[i])
        if hlt_mutau[i] and trig_match_loose[i]:
            hist_num_pt_loose.Fill(pt_2[i])
        if hlt_mutau[i] and trig_match_deep[i]:
            hist_num_pt_deep.Fill(pt_2[i])
...
def make_efficiency_hist(numerator, denominator, name):
    ...
    for i in range(1, hist.GetNbinsX() + 1):
        ...
        err = sqrt(eff * (1 - eff) / den) if den > 0 else 0
...
output_path = os.path.join(root_dir, f"combined_efficiency_pt_{root_filename}.png")
