...
parser = argparse.ArgumentParser(description="Trigger Efficiency Plotter")
...
for i in range(len(eta_2)):
    hist_total_eta_1.Fill(eta_2[i])
    hist_total_pt_1.Fill(pt_2[i])
    
    if hlt_mu24[i] and (trig_match_deep[i] or trig_match_loose[i] or trig_match_medium[i] or trig_match_tight[i]):
        hist_num_eta_1.Fill(eta_2[i])
        hist_num_pt_1.Fill(pt_2[i])

    if hlt_24[i]:
        hist_total_eta_2.Fill(eta_2[i])
        hist_total_pt_2.Fill(pt_2[i])
        if trig_match_deep[i] or trig_match_loose[i] or trig_match_medium[i] or trig_match_tight[i]:
            hist_num_eta_2.Fill(eta_2[i])
            hist_num_pt_2.Fill(pt_2[i])
...
eff_1_eta_plot_path = os.path.join(root_dir, f"efficiency_1_eta_{root_filename}.png")
eff_1_pt_plot_path = os.path.join(root_dir, f"efficiency_1_pt_{root_filename}.png")
eff_2_eta_plot_path = os.path.join(root_dir, f"efficiency_2_eta_{root_filename}.png")
eff_2_pt_plot_path = os.path.join(root_dir, f"efficiency_2_pt_{root_filename}.png")

create_root_plot(eff_1_eta, "Efficiency 1: HLT_IsoMu24_eta2p1", "#tau #eta", "HLT_IsoMu24_eta2p1", eff_1_eta_plot_path)
create_root_plot(eff_1_pt, "Efficiency 1: HLT_IsoMu24_eta2p1", "#tau p_{T} (GeV)", "HLT_IsoMu24_eta2p1", eff_1_pt_plot_path)
create_root_plot(eff_2_eta, "Efficiency 2: HLT_IsoMu24 & HLT_IsoMu20_eta2p1_CrossL1", "#tau #eta", "HLT_IsoMu24 + HLT_IsoMu20_eta2p1_CrossL1", eff_2_eta_plot_path)
create_root_plot(eff_2_pt, "Efficiency 2: HLT_IsoMu24 & HLT_IsoMu20_eta2p1_CrossL1", "#tau p_{T} (GeV)", "HLT_IsoMu24 + HLT_IsoMu20_eta2p1_CrossL1", eff_2_pt_plot_path)
