...
PT_CUTS = {"mutau":30,"ditau":35,"ditaujet":50,"vbfditau":30,"vbfsingletau":50,"etau":35}
...
def plot_data_mc_ratio(eff_data, eff_mc, ratio, x_range, x_title, ytitle, trig_display, algo_display, dm_tag, save_base, pt_cut=None, show_pt_cut=False):
    ...
    out_root = ROOT.TFile(save_base + ".root", "UPDATE")
    for h in [eff_data, eff_mc, ratio]:
        h.Sumw2()
        h.SetName(f"{h.GetName()}_{'data' if h==eff_data else 'mc' if h==eff_mc else 'ratio'}")
        h.Write()
        h_err = h.Clone(f"{h.GetName()}_errors")
        h_err.Reset()
        for ib in range(1, h.GetNbinsX() + 1):
            h_err.SetBinContent(ib, h.GetBinError(ib))
        h_err.SetTitle(f"Bin errors for {h.GetName()}")
        h_err.Write()
    out_root.Close()
    canvas.Close()
