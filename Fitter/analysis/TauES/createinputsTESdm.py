...
#              In particular, to apply the tauID SF vs DM on the fly
...
def main(args):
  ...
  for era in eras:
    for channel in channels:
      ...
      # GET TauID SF vs DM
      from ROOT import loadTauIDSF
      era_      = '2016Legacy' if '16' in era else '2017ReReco' if '17' in era else '2018ReReco'
      fname     = "/data/lepton/tauID/TauID_SF_dm_DeepTau2017v2p1VSjet_%s_ptgt20.root"%(era_)
      loadTauIDSF(fname,"Medium")
	  addsfs    = [
          "getTauIDSF(dm_2,genmatch_2)",
      ]
      rmsfs     = [ ] if (channel=='mumu' or not notauidsf) else ['idweight_2']  #,'ltfweight_2'] # remove tau ID SFs, but why lft???
      ###############
      #   SAMPLES   #
      ###############
      ...
      sampleset = getsampleset(channel,era,fname=sname,join=join,split=None,table=False,rmsf=rmsfs,addsf=addsfs)
