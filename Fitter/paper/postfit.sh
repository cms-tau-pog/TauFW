...
OBS=`echo $OBS | grep -Po '\b(?<!#)\w+\b' | xargs`
...
for channel in $CHANNELS; do
  ...
  for bin in $BINS; do
    ...
    peval "combine -M FitDiagnostics -n .$BINLABEL $WORKSPACE -m $MASS $FIT_OPTS $XRTD_OPTS $CMIN_OPTS" 2>&1 | tee -a $LOG || exit 1 #> ../$LOG
    echo
