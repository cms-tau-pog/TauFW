...
#            print idx, vals[idx]
    ...
    #leg.SetNColumns(2)
        ...
        ensureDir('plots/pull_pt')
    
        canvas.Print('plots/pull_pt/' + yearwp + ".gif")
        canvas.Print('plots/pull_pt/' + yearwp + ".pdf")
...
#for year in ['2016_preVFP']:
	...
	for iso in ['VVVLoose', 'VVLoose', 'VLoose', 'Loose', 'Medium', 'Tight', 'VTight', 'VVTight']:
            ...
            file = open(jobdir + '/out.' + year + '_' + iso + '_pt' + pt)
...
#                    pt = words[3].replace('pt','')
                    ...
                    pulldirs[year + '_' + iso + '_' + pt] = {'titles':[],
                                                             'vals':[],
                                                             'errs':[]}
                ...
                if line.find('ZTT_mu')!=-1:
                    ...
                    print year, iso, '(pT = ', wprange[pt]['low'], '-', wprange[pt]['up'], ')', '{0:.3f}'.format(val), '+{0:.3f}'.format(errval_up), ' / -{0:.3f}'.format(errval_down)
 

                    print 'SetPoint', graphs[year + '_' + iso]['count'], Double(wpdict[pt]), val
                    graphs[year + '_' + iso]['graph'].SetPoint(graphs[year + '_' + iso]['count'], Double(wpdict[pt]), val)

            
                    graphs[year + '_' + iso]['graph'].SetPointError(graphs[year + '_' + iso]['count'], Double(wpdict[pt]-wprange[pt]['low']), Double(wprange[pt]['up'] - wpdict[pt]), abs(errval_up), abs(errval_down))
                    ...
                    pulldirs[year + '_' + iso + '_' + pt]['titles'].append(words[0])
                    pulldirs[year + '_' + iso + '_' + pt]['vals'].append(words[2])
                    pulldirs[year + '_' + iso + '_' + pt]['errs'].append(words[4])
...
#        print year, wp, '(pT = ', wprange[pt]['low'], '-', wprange[pt]['up'], ')', '{0:.3f}'.format(val), '+{0:.3f}'.format(errval_up), ' / -{0:.3f}'.format(errval_down)
 #       print 'count = ', graphs[year + '_' + wp]['count'], '(x,y) = ', Double(wpdict[wp]), val, '-', val_down, '+', val_up
        ...
        print 'old ... SetPoint', graphs_prev[year + '_' + wp]['count'], Double(wpdict[pt]), val
        graphs_prev[year + '_' + wp]['graph'].SetPoint(graphs_prev[year + '_' + wp]['count'], Double(wpdict[pt]), val)


        graphs_prev[year + '_' + wp]['graph'].SetPointError(graphs_prev[year + '_' + wp]['count'], Double(wpdict[pt]-wprange[pt]['low']), Double(wprange[pt]['up'] - wpdict[pt]), abs(errval_up), abs(errval_down))
...
#for idx, year in enumerate(['2017']):
#for idx, year in enumerate(['2017']):
