...
function main {
  ...
  if [ $doDataCard -gt 0 ]; then
    peval "cd ${WORKDIR}"
    peval "python ./harvestDatacards_TES.py --shift-range ${RANGE} -y $YEAR -o m_vis -d DM0 DM1 DM10 DM11 --tag $TAG -e $EXTRATAG $VERBOS "
    peval "python ./harvestDatacards_TES.py --shift-range ${RANGE} -y $YEAR -o m_2 -d DM1 DM10 DM11 --tag $TAG -e $EXTRATAG $VERBOS "
  fi
  ...
  for DM in $DMS; do
    ...
    for var in $VARS; do
      ...
      if [ $doWorkSpace -gt 0 ]; then
        peval "cd ${OUTDIR}"
        peval "text2workspace.py ${DATACARD} -o ${WORKSPACE}" 2>&1 $VERBOS | tee -a ../$LOG || exit 1
      fi
  
      ################ NLL profile scan ######################
      if [ $doNLLScan -gt 0 ]; then
        peval "cd ${OUTDIR}"
        peval "combine -M MultiDimFit ${WORKSPACE} ${ALGO} ${POI_OPTS} -n .${BINLABEL} ${FIT_OPTS} ${XRTD_OPTS} ${CMIN_OPTS} --saveNLL --saveSpecifiedNuis all" 2>&1 $VERBOS | tee -a ../$LOG || exit 1
      fi
  
      ################ make pull plots #######################
      if [ $doPull -gt 0 ]; then
        peval "cd ${OUTDIR}"
        peval "combineTool.py -M Impacts -n $BINLABEL -d $WORKSPACE $FIT_OPTS --points ${NPOINTS} --redefineSignalPOIs tes $POI_OPTS $XRTD_OPTS $CMIN_OPTS --doInitialFit" 2>&1 $VERBOS | tee -a ../$LOG || exit 1
        peval "combineTool.py -M Impacts -n $BINLABEL -d $WORKSPACE $FIT_OPTS --points ${NPOINTS} --redefineSignalPOIs tes $POI_OPTS $XRTD_OPTS $CMIN_OPTS --doFits --parallel 4" 2>&1 $VERBOS | tee -a ../$LOG || exit 1
        peval "combineTool.py -M Impacts -n $BINLABEL -d $WORKSPACE $FIT_OPTS --points ${NPOINTS} --redefineSignalPOIs tes $POI_OPTS $XRTD_OPTS $CMIN_OPTS -o $OUTPUT_IMPA" 2>&1 $VERBOS | tee -a ../$LOG || exit 1
        peval "plotImpacts.py -i $OUTPUT_IMPA -o ../postfit_${YEAR}/$IMPA_NAME" 2>&1 $VERBOS | tee -a ../$LOG || exit 1
        peval "convert -density 160 -trim ../postfit_${YEAR}/${IMPA_NAME}.pdf[0] -quality 100 ../postfit_${YEAR}/${IMPA_NAME}.png" 2>&1 $VERBOS | tee -a ../$LOG || exit 1
      fi
      ...
      if [ $doPostFit -gt 0 ]; then
        ...
        ################ COMBINE FIT ##########################
          peval "combine -M FitDiagnostics -n .$BINLABELT $WORKSPACE $POI_OPTS2 $FIT_OPTS $XRTD_OPTS $CMIN_OPTS --saveShapes" 2>&1 $VERBOS | tee -a ../$LOGT || exit 1
        
        ################ POSTFIT SHAPES #######################
          echo
          peval "PostFitShapesFromWorkspace -o $SHAPES -f ${FITDIAGN}:fit_s --postfit --freeze tes=$tes --sampling --print -d $DATACARD -w $WORKSPACE" 2>&1 $VERBOS | tee -a ../$LOGT || exit 1
        
        ################ DRAW #################################
          peval "cd ${WORKDIR}"
          peval "python checkShapes_TES.py $OUTDIR/$SHAPES --postfit --pdf --out-dir $OUTDIR_PF --dirname $DM -t $TESTAG -y ${YEAR}" 2>&1 $VERBOS | tee -a $LOGT || exit 1
        
        ################ PULLS ################################
          echo
          peval "python $PULL --vtol=0.0001 $OUTDIR/$FITDIAGN | sed 's/[!,]/ /g' | tail -n +3 > $PULLT" 2>&1 $VERBOS | tee -a $LOGT || exit 1
          peval "python $PULLS -f $PULLT -o ${OUTDIR_PF}/pulls_${BINLABELT} -t '$var, $DMLABEL, $PVAL TES'" 2>&1 $VERBOS | tee -a $LOGT || exit 1
          #peval "python $PULLS -b -f $PULLT -o ${OUTDIR_PF}/pulls_${BINLABELT} -t '$var, $DMLABEL, $PVAL TES'" 2>&1 $VERBOS | tee -a $LOGT || exit 1
  ...
  if [ $doParabola -gt 0 -a $doPostFit -gt 0 ]; then
    peval "cd $WORKDIR"
    for var in $VARS; do
      peval "python plotParabola_TES.py -y $YEAR -t $TAG -d $DMS -e $EXTRATAG -a -o $var -r $RANGE" 2>&1 $VERBOS | tee -a $LOG #|| exit 1
      peval "python plotParabola_TES.py -y $YEAR -t $TAG -d $DMS -e $EXTRATAG -r $RANGE -s" 2>&1 $VERBOS | tee -a $LOG #|| exit 1
      peval "python plotPostFitScan_TES.py -y $YEAR -t $TAG -d $DMS -e $EXTRATAG -o $var -r $RANGE" 2>&1 $VERBOS | tee -a $LOG #|| exit 1
    done
  fi
