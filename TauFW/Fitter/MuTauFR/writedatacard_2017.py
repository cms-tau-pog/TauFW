...
for ieta in eta :
    ...
    for iwp in wp :
        ...
        cb.AddObservations(['*'], ['MuTauFR'], ['2017'], ['mutau'],              categories['mutau']) # adding observed data
        cb.AddProcesses(   ['*'], ['MuTauFR'], ['2017'], ['mutau'], backgrounds, categories['mutau'], False) # adding backgrounds
        cb.AddProcesses(   ['*'], ['MuTauFR'], ['2017'], ['mutau'], signals,     categories['mutau'], True) # adding signals

        cb.cp().process(mc_backgrounds).AddSyst(cb,'lumi_2017', 'lnN', ch.SystMap()(1.026))
        ...
        filepath = os.path.join(os.environ['CMSSW_BASE'],'src/TauFW/Fitter/MuTauFR/input', "MuTauFR_m_vis_eta%s_mt-2017.inputs.root")%(ieta)
        ...
        datacardPath = 'input/2017/MuTauFR/%s_eta%s.txt'%(iwp,ieta)
        shapePath = 'input/2017/MuTauFR/common/%s_eta%s.root'%(iwp,ieta)
