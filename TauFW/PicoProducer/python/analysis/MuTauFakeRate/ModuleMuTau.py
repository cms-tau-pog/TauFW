...
class ModuleMuTau(ModuleTauPair):
  ...
  def __init__(self, fname, **kwargs):
    ...
    if self.ismc:
      ...
      self.fesTool = TauFESTool(tauSFVersion[self.year]) # for mu -> tau fake negligible
  ...
  def analyze(self, event):
    ...
    for tau in Collection(event,'Tau'):
      ...
      # removing this #if tau.idDeepTau2017v2p1VSe<1: continue # VVVLoose
      # removing this because of FAIL region is needed #if tau.idDeepTau2017v2p1VSmu<1: continue # VLoose
      ...
      if self.ismc:
        ...
        #  tau.mass *= fes
        #  tau.es    = fes
        elif genmatch in [2,4]: # muon -> tau fake (apply by default, override with 'ltf=1.0')
          if self.fes!=None:# user-defined energy scale (for LES studies)
            fes = self.fes
          else:
            fes = self.fesTool.getFES(tau.eta,tau.decayMode,unc=self.fes)
          if fes!=1:
