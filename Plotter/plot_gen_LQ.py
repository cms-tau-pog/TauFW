...
ht     = "H_{#lower[-0.15]{T}} = #lower[-0.40]{#scale[0.68]{#sum}}#kern[0.009]{#lower[-0.09]{%s (%s > 20 GeV, |#eta| < 4.7) [GeV]}}"%(jpt,jpt)
...
#   ###Var('ntgen',       7,    0,    7, title="Number of top quarks", ),
  ...
  Var('ntaus20',      7,   0,    7, title="Number of #tau leptons (%s > 20 GeV, |#eta| < 2.5)"%pt ),
  Var('ntaus50',      7,   0,    7, title="Number of #tau leptons (%s > 50 GeV, |#eta| < 2.5)"%pt ),
  ...
  Var('eta_tau1',    24,  -6,    6, title="Leading #tau lepton eta", ymargin=1.46, ncol=2, pos='LL' ),
  ###Var('eta_tau1',         bins_eta, title="Leading #tau lepton eta", ymargin=1.46, ncol=2, pos='LL', fname="$VAR_rebin" ),
  Var('eta_tau2',    24,  -6,    6, title="Subleading #tau lepton eta", ymargin=1.46, ncol=2, pos='LL' ),
  ###Var('eta_tau2',         bins_eta, title="Subleading #tau lepton eta", ymargin=1.46, ncol=2, pos='LL', fname="$VAR_rebin" ),
  Var('eta_vistau1', 24,  -6,    6, title="Leading vis. #tau lepton eta", ymargin=1.46, ncol=2, pos='LL' ),
  Var('eta_vistau2', 24,  -6,    6, title="Sublead. vis. #tau lepton eta", ymargin=1.46, ncol=2, pos='LL' ),
  ...
  Var('mvis_tt',     20,   0, 1200, title="m^{#lower[0.15]{vis}}_{#lower[-0.06]{#tau#tau}}", units='GeV', fname="m_tt_1200", only=['tau'] ),
  Var('mvis_tt',     30,   0, 3000, title="m^{#lower[0.15]{vis}}_{#lower[-0.06]{#tau#tau}}", units='GeV', fname="m_tt_3000", only=['tau'] ),
  ...
  Var('dR_tt',             bins_dR, title="#DeltaR_{#lower[-0.15]{#tau#tau}}", ymargin=1.46, ncol=2, pos='LL' ),
#   Var('deta_ll',          bins_eta, title="#Delta#eta_{#lower[-0.25]{#tau#tau}}", ymargin=1.46, ncol=2, pos='LL' ),
#   Var('dphi_ll', 15, -3.142, 3.142, title="#Delta#phi_{#lower[-0.25]{#tau#tau}}", ymargin=1.2, pos='Cy=0.85' ),
  ...
  Var('njets',        9,   0,    9, title=f"{njets}", ymargin=1.46, ncol=2 ),
  Var('njets50',      9,   0,    9, title=f"{njets} ({pt} > 50 GeV, |eta| < 4.7)" ),
  ...
  Var('eta_jet1',    24,  -6,    6, title="Leading gen. jet eta", ymargin=1.46, ncol=2, pos='LL' ),
  ###Var('eta_jet1',         bins_eta, title="Leading gen. jet eta", ymargin=1.46, ncol=2, pos='LL', fname="$VAR_rebin" ),
  #Var('pt_jet1',     20,   0,  400, title="Subleading gen. jet "+pt, fname="$VAR_400"  ),
  Var('pt_jet1',     30,   0, 1200, title="Sublead. gen. jet "+pt, fname="$VAR_1200" ),
  Var('pt_jet1',     40,   0, 2000, title="Sublead. gen. jet "+pt, fname="$VAR_2000" ),
  Var('eta_jet2',    24,  -6,    6, title="Sublead. gen. jet eta", ymargin=1.46, ncol=2, pos='LL' ),
  ###Var('eta_jet2',         bins_eta, title="Sublead. gen. jet eta", ymargin=1.46, ncol=2, pos='LL', fname="$VAR_rebin" ),
...
def compare_LQ(name,title,samples,tag="",**kwargs):
  ...
  if 'LQ' in name:
    selections = [
      ...
      Sel('2t-semilep-ptgt20',"pt_tau1>50 && pt_tau2>50 && dm_tt==2", title="#tau#tau -> l#tau_{h}, p_{T}^{#tau} > 50 GeV",veto=['jet']),
      #Sel('2t-fullhad-ptgt20',"pt_tau1>50 && pt_tau2>50 && dm_tt==3", title="#tau#tau -> #tau_{h}#tau_{h}, p_{T}^{#tau} > 50 GeV",veto=['jet']),
    ]
  else:
    selections = [
      Sel('nocuts',            "", title="no cuts", veto=tvars ),
...
def main(args):
  ...
  ## COMPARE pair HNL
  #compare_LQ('HNL-p',"Z' -> NN",[
  #   makemcsample('HNL','ZptoNN_EEJJJJ_Zprime2000_N100_WR5000_NLO', "2/0.1 TeV",1,era,verb=verbosity+2),
  #   makemcsample('HNL','ZptoNN_EEJJJJ_Zprime2000_N500_WR5000_NLO', "2/0.5 TeV",1,era,verb=verbosity+2),
  #   makemcsample('HNL','ZptoNN_EEJJJJ_Zprime4000_N100_WR5000_NLO', "4/0.1 TeV",1,era,verb=verbosity+2),
  #   makemcsample('HNL','ZptoNN_EEJJJJ_Zprime4000_N500_WR5000_NLO', "4/0.5 TeV",1,era,verb=verbosity+2),
  #   makemcsample('HNL','ZptoNN_EEJJJJ_Zprime4000_N1500_WR5000_NLO',"4/1.5 TeV",1,era,verb=verbosity+2),
  #],tag=tag,outdir=outdir,ext=exts,verb=verbosity)
