  ...
  for selection in hists:
    for variable in hists[selection]:
      ...
      if sampleset.datasample:
        ...
        stack.draw(ratio=True)
        stack.drawlegend()
        stack.drawtext("Test SampleSet.gethists_rdf RDataFrame",selection.title)
        ...
        plot.draw(ratio=True)
        plot.drawlegend()
        plot.drawtext("Test SampleSet.gethists_rdf RDataFrame",selection.title)
        plot.saveas(f"{outdir}/testRDF_plot_{variable.filename}_{selection.filename}{tag}_RDF.png")
        plot.close()
  

def test_getrdframe(samples,variables,selections,outdir="plots/test",split=False,tag="",rungraphs=True,ncores=4,verb=0):
  ...
  for selection, variable, samples, hists in res_dict.iterhists():
    ...
    plot.draw(ratio=True)
    plot.drawlegend()
    plot.drawtext("Test RDataFrame",selection.title)
...
if __name__ == "__main__":
  ...
  parser.add_argument('-c', '--ncores',   default=4, type=int, help="number of cores/threads, default=%(default)s" )
