...
def unpack_gethist2D_args(*args,**kwargs):
  ...
  if len(vargs)==1:
    ...
    if isinstance(xvars,Variable) and isinstance(yvars,Variable): # one pair of Variable objects
      varlist = [(xvars,yvars)]
      singlevar = True
    elif islist(xvars) and islist(yvars): # one pair of lists of Variable objects
      if type(xvars)==type(yvars) and all(isinstance(v,Variable) for v in xvars+yvars): # assume two lists of Variable objects
        varlist = zip(xvars,yvars)
      elif all(islist(x) for x in xvars) and all(islist(y) for y in yvars): # convert to list of pairs of Variable objects
        varlist = [(Variable(*x),Variable(*y)) for x, y in zip(xvars,yvars)]
      elif len(xvars) in [2,4] and len(yvars) in [2,4] and isinstance(xvars[0],str) and isinstance(yvars[0],str): # convert to list of one pair of Variable objects
